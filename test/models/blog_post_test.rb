...
class BlogPostTest < ActiveSupport::TestCase
  ...
  test "pulblished? returns true for published blog post" do
    assert blog_posts(:published).published?
  end

  test "pulblished? returns false for draft blog post" do
    refute blog_posts(:draft).published?
  end

  test "pulblished? returns false for scheduled blog post" do
